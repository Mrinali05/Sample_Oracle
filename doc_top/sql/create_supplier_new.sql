...
AS
   ...
   BEGIN
      ...
      write_log (
            '******Program starts to create suppliers for an employee or a contingent worker...'
      ...
      INSERT INTO XXOYO_EMP_AS_VENDOR_TBL (REQUEST_ID,
         ...
         (SELECT l_request_id,
                 ...
                 papf.full_name,
                 paaf.ass_attribute2 legal_entity,
                --old  papf.current_employee_flag 

                 NVL (papf.current_employee_flag,'C')
                                           current_employee_flag,
                 hru.name,
                 hru.organization_id,
                 -- old (papf.employee_number
           ...
           WHERE     papf.person_id = paaf.person_id
               ...
               ---  AND papf.npw_number IS  NOT NULL        -- Only Employees are to be considered
                 ...
                 AND
               --  old papf.current_employee_flag = 
                 NVL (papf.current_employee_flag, papf.current_npw_flag) =
                                                 'Y'
           ...
           WHERE     papf.person_id = paaf.person_id
              ...
              ---AND papf.npw_number IS  NOT NULL        -- Only Employees are to be considered
                 ...
                 AND TRUNC (SYSDATE) BETWEEN paaf.effective_start_date
                                         AND NVL (paaf.effective_end_date,
                                                  SYSDATE)
               -- old  AND papf.current_employee_flag =
                AND  NVL (papf.current_employee_flag, papf.current_npw_flag) =
                                                 'Y'
           ...
           WHERE     papf.person_id = paaf.person_id
             ...
             --- AND papf.npw_number IS  NOT NULL        -- Only Employees are to be considered
                 ...
                 AND TRUNC (SYSDATE) BETWEEN paaf.effective_start_date
                                         AND NVL (paaf.effective_end_date,
                                                  SYSDATE)
               --  AND papf.current_employee_flag = 
                 AND NVL (papf.current_employee_flag, papf.current_npw_flag) =
                                                 'Y'
      ...
      LOOP
         ...
         write_log ('');
         write_log (
               '****** Starting supplier interface load for employee and contingent_worker# : '
      ...
      write_log ('');
      write_log (
            'Program completes to create suppliers from employee and contingent worker...'
         || TO_CHAR (SYSDATE, 'DD-MON-YYYY HH24:MI:SS'));
   ...
   EXCEPTION
      ...
      THEN
         ROLLBACK;
         p_out_ret_msg :=
               'Unexpected error occurred while creating Vendor of an Employee and Contingent worker, '
            || SUBSTR (SQLERRM, 1, 500);
         p_out_ret_code := 2;
